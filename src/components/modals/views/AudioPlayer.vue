<template>
<el-dialog :title="lang.modal.audioPlayer.title" ref="fmModal" :visible.sync="showModal" width="30%" :before-close="hideModal">

    <div class="modal-content fm-modal-audio-player">
        <div class="modal-body">
            <audio ref="fmAudio" controls />
            <hr>
            <div class="d-flex justify-content-between py-2 px-2" v-bind:class="playingIndex === index ? 'bg-light' : ''" v-for="(item, index) in audioFiles" v-bind:key="index">
                ...
                <template v-if="playingIndex === index">
                    <div v-if="status === 'playing'">
                        <i v-on:click="togglePlay()" class="far fa-play active" />
                    </div>
                    <div v-else>
                        <i v-on:click="togglePlay()" class="far fa-pause" />
                    </div>
                </template>
                <template v-else>
                    <div>
                        <i v-on:click="selectTrack(index)" class="far fa-play" />
                    </div>
                </template>
            </div>
        </div>
